          ...
          if occurrences==1 :
            for i in player_board["P2"]:
                ...
                if (king_finder()["P1"] in movements) and (move_to ==i):
                    p1_check=False
                    break
                mto=board[move_to[0]][int(move_to[1])]
                move(move_from,move_to)
                player_board=players_boards()
                p2_list_all=[]
                for i in player_board["P2"]:
                    p2_list_all=p2_list_all+available_cells("P1",i)
                if king_finder()["P1"] not in p2_list_all:
                    move(move_to,move_from)
                    board[move_to[0]][int(move_to[1])]=mto
                    p1_check=False 
          if not p1_check:
        ...
        if move_to in available_cells(player,move_from):
          ...
          if occurrences==1 :
            for i in player_board["P1"]:
                ...
                for i in player_board["P1"]:
                    p1_list_all=p1_list_all+available_cells("P1",i)
                if king_finder()["P2"] not in p1_list_all:
                    move(move_to,move_from)
                    board[move_to[0]][int(move_to[1])]=mto
                    p2_check=False   
          if not p2_check:   
        ...
        if "⚔️" in piece:
            ...
            for i in range(ord(move_from_l)+1,105):
                if "P1" not in board[chr(i)][move_from_n]:
                    if "P2" in board[chr(i)][move_from_n]:
            ...
            for i in range(move_from_n+1,9):
                if f_counter<=104:
                    if "P1" not in board[chr(f_counter)][i]:#allowed_movements.append(chr(f_counter)+str(i))
                        if "P2" in board[chr(f_counter)][i]:
            ...
            for i in range(move_from_n-1,0,-1):
                if s_counter>=97:
                    if "P1" not in board[chr(s_counter)][i]:
                        if "P2" in board[chr(s_counter)][i]:
        ...
        if "⚔️" in piece:
            ...
            for i in range(ord(move_from_l)+1,105):
                if "P2" not in board[chr(i)][move_from_n]:
                    if "P1" in board[chr(i)][move_from_n]:
                        allowed_movements.append(chr(i)+str(move_from_n))
                        break
                    else:
                        allowed_movements.append(chr(i)+str(move_from_n))
            ...
            for i in range(move_from_n+1,9):
                if f_counter<=104:
                    if "P2" not in board[chr(f_counter)][i]:#allowed_movements.append(chr(f_counter)+str(i))
                        if "P1" in board[chr(f_counter)][i]:
                            allowed_movements.append(chr(f_counter)+str(i))
                            break
                        else:
                            allowed_movements.append(chr(f_counter)+str(i))
            ...
            for i in range(move_from_n-1,0,-1):
                if s_counter>=97:
                    if "P2" not in board[chr(s_counter)][i]:
                        if "P1" in board[chr(s_counter)][i]:
                            allowed_movements.append(chr(s_counter)+str(i))
                            break
                        else:
                            allowed_movements.append(chr(s_counter)+str(i))
            ...
            for i in range(move_from_n-1,0,-1):
                if s_counter<=104:#s +
                    if "P2" not in board[chr(s_counter)][i]and (s_counter<=104):
                        if "P1" in board[chr(s_counter)][i]:
                            allowed_movements.append(chr(s_counter)+str(i))
                            break
                        else:
